...
if len(STORIES) != 1910:
    raise ValueError(f"Expected 1,910 stories, found {len(STORIES)}")

new_audit_rows = [audit_story(story) for story in STORIES[BASE_COUNT:]]
if AUDIT_ISSUES:
    raise ValueError("Stories exceed guardrails:\n" + "\n".join(AUDIT_ISSUES))
old_audit_rows = read_csv("vocabulary_audit_1810.csv")
audit_fields = list(old_audit_rows[0])
write_csv("vocabulary_audit_1910.csv", old_audit_rows + new_audit_rows, audit_fields)

old_review_rows = read_csv("vocabulary_editorial_review_1810.csv")
new_review_rows = [{
    "id": story["id"], "level": story["level"], "topic": story["topic"],
...
write_csv("vocabulary_editorial_review_1910.csv", old_review_rows + new_review_rows, review_fields)
