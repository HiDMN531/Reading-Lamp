...
def probable_proper_nouns(sentences):
    ...
    return {
        ...
        if word in capitalized_noninitial
        or (count >= 2 and zipf_frequency(word, "en") < 4)
    }


def audit_story(story):
    sentences = [
        WORD_RE.findall(part)
        for part in SENTENCE_RE.split(story["text"])
    ...
    syntax_flag = average_sentence > syntax_limit
    if lexical_flags or syntax_flag:
        raise ValueError(
            f'{story["id"]} exceeds guardrails: lexical={lexical_flags}, '
            f"avg_sentence={average_sentence:.2f}"
        )
    return {
        "id": story["id"],
        "level": story["level"],
        ...
        "avg_sentence_words": round(average_sentence, 2),
        "mean_zipf_frequency": round(sum(frequencies) / len(frequencies), 3),
        "low_frequency_pct": round(low, 2),
        "very_low_frequency_pct": round(very_low, 2),
        "long_word_pct": round(long_words, 2),
        "probable_proper_noun_types": len(proper_nouns),
        "lexical_flags": "",
        "syntax_flag": "no",
...
if len(STORIES) != 1710:
    raise ValueError(f"Expected 1,710 stories, found {len(STORIES)}")

new_audit_rows = [audit_story(story) for story in STORIES[BASE_COUNT:]]
old_audit_rows = read_csv("vocabulary_audit_1610.csv")
audit_fields = list(old_audit_rows[0])
write_csv("vocabulary_audit_1710.csv", old_audit_rows + new_audit_rows, audit_fields)

old_review_rows = read_csv("vocabulary_editorial_review_1610.csv")
new_review_rows = [
    {
        "id": story["id"],
        "level": story["level"],
        "topic": story["topic"],
        "title": story["title"],
        "metric_status": "within-guardrails",
        "lexical_flags": "",
        "editorial_result": "pass",
        "action": "no-further-change",
        "evidence_terms": "",
        "editorial_note": "Level別の語彙・構文ガードレール内。",
    }
    for story in STORIES[BASE_COUNT:]
]
review_fields = list(old_review_rows[0])
write_csv(
    "vocabulary_editorial_review_1710.csv",
    old_review_rows + new_review_rows,
    review_fields,
)
...
print(
    json.dumps(
        {
            "stories": len(STORIES),
            "newStories": len(new_audit_rows),
            "newWithinGuardrails": len(new_audit_rows),
            "newReviewRequired": 0,
        },
        ensure_ascii=False,
    )
