...
if len(STORIES) != 2000:
    raise ValueError(f"Expected 2,000 stories, found {len(STORIES)}")

new_audit_rows = [audit_story(story) for story in STORIES[BASE_COUNT:]]
if AUDIT_ISSUES:
    raise ValueError("Stories exceed guardrails:\n" + "\n".join(AUDIT_ISSUES))
old_audit_rows = read_csv("vocabulary_audit_1910.csv")
audit_fields = list(old_audit_rows[0])
write_csv("vocabulary_audit_2000.csv", old_audit_rows + new_audit_rows, audit_fields)

old_review_rows = read_csv("vocabulary_editorial_review_1910.csv")
new_review_rows = [{
    "id": story["id"], "level": story["level"], "topic": story["topic"],
    ...
    "editorial_note": "Level別の語彙・構文ガードレール内。事実確認・文法確認も完了。",
} for story in STORIES[BASE_COUNT:]]
review_fields = list(old_review_rows[0])
write_csv("vocabulary_editorial_review_2000.csv", old_review_rows + new_review_rows, review_fields)
